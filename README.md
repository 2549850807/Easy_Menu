...
* **2026.2.11-V3.2.0**：1、为条目增加了一个`变量名`属性，用户可以自定义条目的变量名，当变量名重复时，可以勾选 `以父级作为前缀` 防止变量名重复。2、改进了代码的增量更新功能，即使变量名发生改变，回调函数的内容仍然会继承到新的回调函数中。3、优化生成代码的样式，将原先的用 `_` 连接改为 `__` ，以便区分所属页面。
